    ...
    <div className="max-w-[296px] min-height-[444px] gap-1 cursor-pointer relative">
      ...
      <div className="flex items-center  gap-1">
        {[...Array(5)].map((_, index) => (
          <StarIcon
            key={index}
            className={`w-[18.49px] h-[18.49px] ${index < Math.floor(product.rating || 4) ? 'text-[#FFC633]' : 'text-gray-300'}`}
          />
        ))}
        <span className="ABeeZee lg:text-[14px] sm:text-[12px] text-gray-600 ml-1">{product.rating || "4.5/5"}</span>
      </div>
      ...
      {/* Options Menu Button */}
      <button
        className="absolute top-0 right-2 text-gray-600 p-2 rounded-full"
        onClick={toggleMenu}
      >
        ...
      </button>
      ...
      {isMenuOpen && (
        <div className="absolute top-10 right-2 bg-white border border-gray-300 rounded-lg shadow-md w-36">
          <ul>
            <li className="ABeeZee px-2 py-2 cursor-pointer hover:bg-gray-100" onClick={() => onDelete(product.id)}>Delete</li>
            <li className="ABeeZee px-2 py-2 cursor-pointer hover:bg-gray-100" onClick={() => setIsMenuOpen(false)}>Cancel</li>
          </ul>
        </div>
      )}
    </div>
