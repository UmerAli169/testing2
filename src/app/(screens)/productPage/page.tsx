...
import { Star, MoreHorizontalIcon } from 'lucide-react';
...
const ProductDetails = () => {
  ...
  const ProductCard = ({ product, onClick, onDelete }: any) => {
    ...
    return (
      <div className='max-w-[296px] max-height-[444px] mt-[30px] rounded-lg p-4 cursor-pointer relative' onClick={onClick}>
        <button
        ...
        <div className='flex items-center gap-1'>
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className='h-4 w-4'
              color={i < roundedRating ? '#FACC15' : '#D1D5DB'}
  ...
  return (
    <div className='mx-[100px]'>
      ...
      <div className='flex items-center space-x-2 text-sm text-gray-500 py-5'>
        ...
        <img src='/svgs/productTopBar/replaceOFSlash.svg' alt='/' className='w-[6px]' />
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 lg:mx-[100px] sm:mx-[20px]'>
        {topSelling.slice(0, 4).map((product) => (
          <ProductCard key={product.id} product={product} onClick={() => handleClick(product.id)} onDelete={handleDelete} />
        ))}
      </div>
    </div>
  );
